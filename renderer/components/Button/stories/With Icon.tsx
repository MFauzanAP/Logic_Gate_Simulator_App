...
WithIcon.parameters = {
	docs		: {
		storyDescription	: `
You can add icons to your button by passing in the name of the icon to either the \`startIcon\` or \`endIcon\` prop.

Icon styles and props can be passed through the \`startIconProps\` or \`endIconProps\` prop
`,
		source				: {
			expanded			: true,
			language			: 'jsx',
...
{/* Default Button */}
<Button label={'Enable'} startIcon={'power'} />

{/* Primary Button */}
<PrimaryButton label={'Confirm'} endIcon={'checkmark'} />

{/* Secondary Button */}
<SecondaryButton label={'Close'} endIcon={'close'} />
			`,
		},
	},
};
...
WithIcon.play = async () => {
	...
	await expect(defaultButton).toHaveTextContent('Enable');
	...
	await expect(primaryButton).toHaveTextContent('Confirm');
	...
	await expect(secondaryButton).toHaveTextContent('Close');
